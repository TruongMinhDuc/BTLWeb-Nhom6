...
const upDate = async (req, res) => {
  ...
  try {
    ...
    const isDuplicate = req.body.teller.some((item) => {
      ...
      return findGathering.teller.some((teller) =>
        teller.idTeller.equals(itemObjectId)
      );
    });
    ...
    if (isDuplicate) {
      return res.status(400).json({
        message:
          "Các giá trị trong req.body.teller đã tồn tại trong findGathering.teller",
      });
    }
    ...
    return res
      .status(200)
      .json({ message: "Thêm thành công", data: updatedGathering });
  } catch (error) {
...
const createNoteandAddtoGathering = async (req, res) => {
  try {
    const id = req.params.id;
    const { name, address, phone, type, Date, Status, station, typeNote } =
      req.body;
...
const sendTotransactionPoint = async (req, res) => {
  try {
    ...
    const { _id, name, address, phone, type, Status, station, typeNote } =
      req.body;
...
const deleteId = async (req, res) => {
  try {
    const id = req.params.id;
    const idGathering = req.params.gathering;
    const deleteNote = await Gathering.updateOne(
      {
        _id: idGathering,
      },
