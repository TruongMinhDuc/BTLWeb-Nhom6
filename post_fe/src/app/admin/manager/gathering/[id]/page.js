...
import { find, updateNote } from "@/service/NOTE";
...
function Gathering({ params }) {
  ...
  async function send(idNote) {
    const { Status, address, name, phone, station, type, typeNote } =
      submitValue;
  ...
  async function removeN(id) {
    const res = await removeNote(id, params.id);
    if (res?.message == "Xóa thành công") {
      toast.success(res.message, { theme: "dark", position: "top-center" });
      window.location.reload();
    } else {
      toast.error(res.message, { theme: "dark", position: "top-center" });
    }
  }
  return (
    <section className="w-full">
      ...
      <div className="p-5 flex space-x-20 items-center">
        <h2 className="text-xl">
          Thông tin người quản lý khu : {gathering?.name}
        </h2>
                            ...
                            >
                              ...
                              <section className="w-[500px] p-4">
                                ...
                                <div>
                                  ...
                                  <select
                                    name="typeNote"
                                    onChange={(e) => setValueSendNote(e.target)}
                                  >
                                ...
                                >
                                  ...
                                  {gathering?.transactions?.map((item) => {
                                    return (
                                      <option value={item.idTransaction}>
                                        {item.pointName}
                                      </option>
                                    );
                                  })}
                            ...
                            >
                              ...
                              <section className="w-[500px] p-4">
                                ...
                                <select
                                  ...
                                  onChange={(e) => setValueSendNote(e.target)}
                                >
                                  <option>--Điểm giao dịch Tập kết--</option>
                                  {gatherings?.map((item) => {
                                    return (
                                ...
                                <Button
                                  className="p-4"
                                  onClick={() => sendToGathering(item?._id)}
                                >
                                  Gửi
                                </Button>
                              </section>
                                  ...
                                  <div className="">
                                    ...
                                    <div className="p-4">
                                      ...
                                      <p>
                                        ...
                                        phiếu gửi và cam đoan bưu gửi này không
                                        chứa những hàng nguy hiểm, những hàng
                                        cấm.Trường hợp không phát được tôi sẽ
                                        thanh toán tiền
                                      </p>
                                    </div>
                                    <div className="border border-l border-black"></div>
